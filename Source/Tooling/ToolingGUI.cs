        ...
        public UITab RenderToolingTab()
        {
            ...
            if (_isToolingTempDisabled || _untooledParts.Count > 0)
            {
                ...
                foreach (UntooledPart up in _untooledParts)
                {
                    ...
                    GUILayout.Label($"{up.ToolingCost:N0}f", RightLabel, UIHolder.Width(72));
                    float untooledExtraCost = GetUntooledExtraCost(up);
                    GUILayout.Label($"{up.TotalCost:N0}f", RightLabel, UIHolder.Width(72));
                    GUILayout.Label($"{(up.TotalCost - untooledExtraCost):N0}f", RightLabel, UIHolder.Width(72));
                    GUILayout.EndHorizontal();
                }
